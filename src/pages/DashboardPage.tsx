...
export const DashboardPage: React.FC = () => {
    ...
    weekago.setDate(weekago.getDay() -1)
    const [data, setData] = React.useState([])
    React.useEffect(() => {
        getEntriesByDateRange({start: now, end: weekago}).then((res) => {
            console.log('res', res)
    ...
    return (
        <Box>
            ...
            {data.map((d) => <div>{d.desc}</div>)}
            <StackedBarChart categoryIds={list} data={data}/>
        </Box>
    )
}