        ...
        return (
            <li key={c.categoryId}>
                <ul>
                    ...
                    {c.children?.map((c) =>
                        <ListItem onClick={() => addCategory(categoryIdMap[c])} key={c}><Chip icon={selectedCategory === categoryIdMap[c]?.categoryName ? <CheckCircleIcon /> : <></>} sx={{ ml: 3, background: categoryIdMap[c]?.color }} label={categoryIdMap[c]?.categoryName} /></ListItem>)}
                </ul>
            </li>
        )
